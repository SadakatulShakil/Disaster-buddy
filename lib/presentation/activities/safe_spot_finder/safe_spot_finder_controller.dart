...
/// narration) without re-deriving it itself.
...
/// (once).
class SafeSpotFinderController extends GetxController {
  ...
  /// Feedback text from the most recently tapped spot (safe or unsafe),
  /// shown until the next tap or scene change.
  final Rx<LocalizedText?> lastFeedback = Rx<LocalizedText?>(null);
  ...
  Future<SafeSpotTapOutcome> handleTap(SafeSpotHotspot spot) async {
    if (!spot.isSafe) {
      ...
      unsafeNonce.value++;
      lastFeedback.value = spot.feedback;
      narrate(spot.feedback);
